import { createComponent, ssr, ssrHydrationKey, ssrStyleProperty, escape, isServer, getRequestEvent, delegateEvents } from 'solid-js/web';
import { n as nu } from '../nitro/nitro.mjs';
...
import { o as oe$1 } from './chunk-ACNSKN6D-CWv6jrK3.mjs';
...
const je = /^(?:[a-z0-9]+:)?\/\//i, We = /^\/+|(\/)\/+$/g, ae = "http://sr";
function B(e, t = false) {
  const r = e.replace(We, "$1");
...
const Ke = 100, Me = createContext(), ue = createContext();
function Ne(e, t = "") {
  ...
  return le(e.path).reduce((c, d) => {
    for (const y of ce(d)) {
      ...
      g = g.split("/").map((b) => b.startsWith(":") || b.startsWith("*") ? b : encodeURIComponent(b)).join("/"), c.push({ ...h, originalPath: d, pattern: g, matcher: $e(g, !i, e.matchFilters) });
    }
    return c;
  }, []);
}
...
function Je(e, t, r, o = {}) {
  const { signal: [n, a], utils: s = {} } = e, i = s.parsePath || ((f) => f), h = s.renderPath || ((f) => f), c = s.beforeLeave || oe(), d = T("", o.base || "");
  ...
  }, [u, l] = createSignal(n().value), [v, w] = createSignal(n().state), A = ze(u, v, s.queryWrapper), S = [], E = createSignal(isServer ? ye() : []), k = createMemo(() => typeof o.transformUrl == "function" ? I(t(), o.transformUrl(A.pathname)) : I(t(), A.pathname)), V = () => {
    const f = k(), p = {};
    for (let R = 0; R < f.length; R++) Object.assign(p, f[R].params);
    return p;
  }, de = s.paramsWrapper ? s.paramsWrapper(V, t) : ie(V), J = { pattern: d, path: () => d, outlet: () => null, resolvePath(f) {
    return T(d, f);
  } };
  return createRenderEffect(on(n, (f) => b("native", f), { defer: true })), { base: J, location: A, params: de, isRouting: y, renderPath: h, parsePath: i, navigatorFactory: pe, matches: k, beforeLeave: c, preloadRoute: we, singleFlight: o.singleFlight === void 0 ? true : o.singleFlight, submissions: E };
  function me(f, p, R) {
    untrack(() => {
      if (typeof p == "number") {
      ...
      const x = !p || p[0] === "?", { replace: j, resolve: O, scroll: W, state: U } = { replace: false, resolve: !x, scroll: true, ...R }, F = O ? f.resolvePath(p) : T(x && A.pathname || "", p);
      if (F === void 0) throw new Error(`Path '${p}' is not a routable path`);
      if (S.length >= Ke) throw new Error("Too many redirects");
      const X = u();
      if (F !== X || U !== v()) if (isServer) {
        const G = getRequestEvent();
        G && (G.response = { status: 302, headers: new Headers({ Location: F }) }), a({ value: F, replace: j, scroll: W, state: U });
      } else c.confirm(F, R) && (S.push({ value: X, replace: j, scroll: W, state: v() }), b("navigate", { value: F, state: U }));
    });
  }
  function pe(f) {
    return f = f || useContext(ue) || J, (p, R) => me(f, p, R);
  }
  function ge(f) {
    const p = S[0];
    p && (a({ ...f, replace: p.replace, scroll: p.scroll }), S.length = 0);
  }
  function we(f, p) {
    const R = I(t(), f.pathname), x = C;
    C = "preload";
    for (let j in R) {
      const { route: O, params: W } = R[j];
...
const fe = (e) => (t) => {
  ...
  return e.create && e.create(s), createComponent(Me.Provider, { value: s, get children() {
...
function st(e = true, t = false, r = "/_server", o) {
  return (n) => {
    ...
    function d(u) {
      ...
      const l = u.composedPath().find((k) => k instanceof Node && k.nodeName.toUpperCase() === "A");
...
function it(e) {
  ...
  return et({ get: t, set({ value: o, replace: n, scroll: a, state: s }) {
    n ? window.history.replaceState(Be(s), "", o) : window.history.pushState(s, "", o), nt(decodeURIComponent(window.location.hash.slice(1)), a), z();
  }, init: (o) => tt(window, "popstate", Ie(o, (n) => {
...
const ct = typeof globalThis.document < "u", ut = ct ? `${location.origin}/api/rivet` : "http://localhost:3000/api/rivet", lt = createClient(ut);
var ht = ["<div", '><nav style="', '"><a href="/">Counter</a> | <a href="/ssr">SSR Counter</a> | <a href="/auth">Auth</a></nav><main style="', '">', "</main></div>"];
function Pt() {
  return createComponent(oe$1, { client: lt, get children() {
    return createComponent(it, { root: (e) => ssr(ht, ssrHydrationKey(), ssrStyleProperty("padding:", "1rem") + ssrStyleProperty(";border-bottom:", "1px solid #ccc"), ssrStyleProperty("padding:", "1rem"), escape(createComponent(Suspense, { get children() {
      return e.children;
    } }))), get children() {
      return createComponent(nu, {});
    } });
  } });
}

export { Pt as default };
//# sourceMappingURL=app-De9iMQfE.mjs.map
